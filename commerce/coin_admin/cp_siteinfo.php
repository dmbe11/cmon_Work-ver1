...
# Code to handle file being loaded by URL
	IF (!eregi('admin.php', $_SERVER['PHP_SELF'])) {
		...
		html_header_location('error.php?err=01&url=admin.php?cp=siteinfo');
		exit;
	}
...
function cp_do_select_form_siteinfo($aaction, $aname, $avalue, $aret_flag=0) {
	...
	# Process query results
		while(list($si_id, $si_group, $si_name, $si_desc, $si_title) = $db_coin->db_fetch_row($result)) {
			$_out .= '<option value="'.$si_id.'">'.$si_group.' - '.$si_name.'</option>'.$_nl;
		}
...
function cp_do_form_add_edit_siteinfo($adata, $aerr_entry, $aret_flag=0) {
	...
	# Do Main Form
		...
		$_cstr .= '<input type="hidden" name="cp" value="siteinfo">'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="si_group" SIZE=32 value="'.htmlspecialchars($adata['si_group']).'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="si_name" SIZE=32 value="'.htmlspecialchars($adata['si_name']).'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="si_seq_no" SIZE=6 value="'.htmlspecialchars($adata['si_seq_no']).'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="si_desc" SIZE=50 value="'.htmlspecialchars($adata['si_desc']).'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="si_title" SIZE="50" value="'.htmlspecialchars($adata['si_title']).'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= '<TEXTAREA class="PSML_NL" NAME="si_code" COLS="'.$_cols.'" ROWS="25">'.$adata['si_code'].'</TEXTAREA>'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
...
function cp_do_display_entry_siteinfo($adata, $aret_flag=0) {
	...
	# Build Title String, Content String, and Footer Menu String
		...
		$_cstr .= '<tr valign="bottom">'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l12_SI_Description'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$adata['si_desc'].'</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= '<tr valign="bottom">'.$_nl;
		$_cstr .= $_td_str_left.'<b>'.$_LANG['_ADMIN']['l12_Title'].$_sp.'</b></td>'.$_nl;
		$_cstr .= $_td_str_right.$adata['si_title'].'</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= '</tr>'.$_nl;
		$_cstr .= '<tr valign="bottom">'.$_nl;
		$_cstr .= $_td_str_colsp2.$adata['si_code'].'</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		$_cstr .= '</table>'.$_nl;
...
IF ($_GPV['op'] == 'none') {
	...
	# Call function for create select form: SiteInfo
		$_cstr = cp_do_select_form_siteinfo($_SERVER["PHP_SELF"].'?cp=siteinfo&op=edit', 'si_id', $_GPV['si_id'], '1');
...
IF ($_GPV['op'] == 'add' && $_GPV['stage'] == 1 && !$err_entry['flag']) {
	# Do select
		...
		$query	.= ' VALUES (';
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_group'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_name'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_seq_no'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_desc'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_block_it'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_title'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_code'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_status'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['si_footer_menu'])."')";
	...
	# Call function to open block
		$_out .= do_subj_block_it($_LANG['_ADMIN']['Add_SiteInfo_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$insert_id.')', '1');
...
IF ($_GPV['op'] == 'edit' && $_GPV['stage'] == 1 && !$err_entry['flag']) {
	# Do select
		$query	 = 'UPDATE '.$_DBCFG['site_info'].' SET ';
		$query	.= "si_group='".$db_coin->db_sanitize_data($_GPV['si_group'])."', ";
		$query	.= "si_name='".$db_coin->db_sanitize_data($_GPV['si_name'])."', ";
		$query	.= "si_seq_no='".$db_coin->db_sanitize_data($_GPV['si_seq_no'])."', ";
		$query	.= "si_desc='".$db_coin->db_sanitize_data($_GPV['si_desc'])."', ";
		$query	.= "si_block_it='".$db_coin->db_sanitize_data($_GPV['si_block_it'])."', ";
		$query	.= "si_title='".$db_coin->db_sanitize_data($_GPV['si_title'])."', ";
		$query	.= "si_code='".$db_coin->db_sanitize_data($_GPV['si_code'])."', ";
		$query	.= "si_status='".$db_coin->db_sanitize_data($_GPV['si_status'])."', ";
		$query	.= "si_footer_menu='".$db_coin->db_sanitize_data($_GPV['si_footer_menu'])."' ";
		$query	.= 'WHERE si_id='.$_GPV['si_id'];
		$result	= $db_coin->db_query_execute($query) OR DIE("Unable to complete request");
	...
	# Call function to open block
		$_out .= do_subj_block_it($_LANG['_ADMIN']['Edit_SiteInfo_Entry_Results'], '1');
