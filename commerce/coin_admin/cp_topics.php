...
# Code to handle file being loaded by URL
	IF (!eregi('admin.php', $_SERVER['PHP_SELF'])) {
		...
		html_header_location('error.php?err=01&url=admin.php?cp=topics');
		exit;
	}
...
function cp_do_select_form_topic($aaction, $aname, $avalue, $aret_flag=0) {
	...
	# Set Query for select.
		$query	= 'SELECT topic_id, topic_name, topic_desc FROM '.$_DBCFG['topics'].' ORDER BY topic_name ASC';
	...
	# Process query results
		while(list($topic_id, $topic_name, $topic_desc) = $db_coin->db_fetch_row($result)) {
			$_out .= '<option value="'.$topic_id.'">'.$topic_name.'</option>'.$_nl;
		}
...
function cp_do_form_add_edit_topic($adata, $aerr_entry, $aret_flag=0) {
	...
	# Do Main Form
		...
		$_cstr .= '<input type="hidden" name="cp" value="topics">'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="topic_name" SIZE=30 value="'.htmlspecialchars($adata['topic_name']).'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= '<INPUT class="PSML_NL" TYPE=TEXT NAME="topic_desc" SIZE=30 value="'.htmlspecialchars($adata['topic_desc']).'">'.$_nl;
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
		...
		$_cstr .= $_td_str_right.$_nl;
		$_cstr .= do_select_list_icon('topic_icon', $adata['topic_icon'], '1');
		$_cstr .= '</td>'.$_nl;
		$_cstr .= '</tr>'.$_nl;
...
IF ($_GPV['op'] == 'view') {
	# Check for valid $_GPV[topic_id] no
		IF ($_GPV['topic_id']) {
		# Set Query for select.
			$query	= 'SELECT * FROM '.$_DBCFG['topics'].' WHERE topic_id='.$_GPV['topic_id'];
			$result	= $db_coin->db_query_execute($query);
			$numrows	= $db_coin->db_query_numrows($result);
...
IF ($_GPV['op'] == 'none') {
	...
	# Call function for create select form: Topics
		$_cstr = cp_do_select_form_topic($_SERVER["PHP_SELF"].'?cp=topics&op=edit', 'topic_id', $_GPV['topic_id'], '1');
...
IF ($_GPV['op'] == 'add' && $_GPV['stage'] == 1 && !$err_entry['flag']) {
	# Do select
		...
		$query	.= ' VALUES (';
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['topic_pos'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['topic_name'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['topic_desc'])."', ";
		$query	.= "'".$db_coin->db_sanitize_data($_GPV['topic_icon'])."')";
	...
	# Call function to open block
		$_out .= do_subj_block_it($_LANG['_ADMIN']['Add_Topics_Entry_Results'].$_sp.'('.$_LANG['_ADMIN']['Inserted_ID'].$_sp.$insert_id.')', '1');
...
IF ($_GPV['op'] == 'edit' && $_GPV['stage'] == 1 && !$err_entry['flag']) {
	# Do select
		$query	 = 'UPDATE '.$_DBCFG['topics'].' SET ';
		$query	.= "topic_pos='".$db_coin->db_sanitize_data($_GPV['topic_pos'])."', ";
		$query	.= "topic_name='".$db_coin->db_sanitize_data($_GPV['topic_name'])."', ";
		$query	.= "topic_desc='".$db_coin->db_sanitize_data($_GPV['topic_desc'])."', ";
		$query	.= "topic_icon='".$db_coin->db_sanitize_data($_GPV['topic_icon'])."' ";
	...
	# Call function to open block
		$_out .= do_subj_block_it($_LANG['_ADMIN']['Edit_Topics_Entry_Results'], '1');
