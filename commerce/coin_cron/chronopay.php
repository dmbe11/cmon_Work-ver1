...
# Silent running, or show progress, and message count(s) on-screen when done
	$ShowProgress = $_ACFG['CHRONOPAY_AUTO_VERBOSE'];
...
# Call import email functions (if enabled)
	IF ($_ACFG['CHRONOPAY_AUTO_IMPORT_ENABLE']) {
	...
	# Connect To Mail Server
		...
		$inbox = @imap_open('{' . $_ACFG['CHRONOPAY_AUTO_IMPORT_SERVER'] . $_ACFG['CHRONOPAY_AUTO_IMPORT_TYPE'] . '}INBOX', $_ACFG['CHRONOPAY_AUTO_IMPORT_USERID'], $_ACFG['CHRONOPAY_AUTO_IMPORT_PASSWORD']);
		...
		# Process The Messages
			...
			# If $_CCFG['DELETE_IMPORTED_MESSAGES'] is ALL then delete message on server
				IF ($_ACFG['CHRONOPAY_AUTO_IMPORT_DELETE_MESSAGES'] == 2) {
			...
			# Process if message is from CHRONOPAY
				IF (strpos($xp_subject_text, $_ACFG['CHRONOPAY_AUTO_PARTIAL_SUBJECT']) === FALSE) {
				...
				# Replace URLencoding
					$xp_body_text = str_replace('=40', '@', $xp_body_text);
					$xp_body_text = str_replace('=24', '$', $xp_body_text);
				...
				# Determine payment vars/values
					...
					foreach($_xpb_parts as $key => $val) {
						...
						# set variables
							IF ($_p2[0] == $_ACFG['CHRONOPAY_AUTO_INVOICE_ID'])	{$ipn->set_txn_invc_id($_p2[1]);}
							IF ($_p2[0] == $_ACFG['CHRONOPAY_AUTO_TRANS_ID'])	{$ipn->set_txn_id($_p2[1]);}
							IF ($_p2[0] == $_ACFG['CHRONOPAY_AUTO_TOTAL_AMT'])	{
								$_p3 = explode(' ', $_p2[1]); 		// Make line into three vars
								$ipn->set_txn_gross($_p3[1]);			// actual amount in numbers
								$ipn->set_txn_currency(trim($_p3[2]));	// Currency Code
							}
...
//	$ipn->set_txn_subscr_id($_GPV['subscr_id']);
				...
				# IF we have a confirmed payment (either test mode or resubmit by admin, or real data from CHRONOPAY), process it
					...
					# Check to make sure recipient is correct
						IF (strpos($ipn->txn_receiver_email, $_CCFG['CHRONOPAY_RECEIVER_EMAIL']) === FALSE) {
					...
					# If $_CCFG['DELETE_IMPORTED_MESSAGES'] is IMPORTED then delete message on server
						IF ($_ACFG['CHRONOPAY_AUTO_IMPORT_DELETE_MESSAGES'] == 1) {
							IF ($ShowProgress) {$_cstr .= '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$_LANG['_IPN']['EMAIL_IMPORT_TO_DELETE'].'<br />';}
							$DeleteIt = imap_delete($inbox, $x);
						}
